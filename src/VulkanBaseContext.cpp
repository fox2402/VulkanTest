...
void VulkanBaseContext::framebufferResizeCallback(GLFWwindow* window, int width, int height) {
	auto app = reinterpret_cast<VulkanBaseContext*>(glfwGetWindowUserPointer(window));
...
void VulkanBaseContext::createSurface()
{
...
bool VulkanBaseContext::checkDeviceExtensionSupport(VkPhysicalDevice device)
{
...
SwapChainSupportDetails VulkanBaseContext::querySwapChainSupport(VkPhysicalDevice device) {
...
VkSurfaceFormatKHR VulkanBaseContext::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR VulkanBaseContext::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
{
...
VkExtent2D VulkanBaseContext::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
{
...
{
	auto vertShaderCode = readFile(ShaderPath() + separator() + "vert.spv");
	auto fragShaderCode = readFile(ShaderPath() + separator() + "frag.spv");
...
VkShaderModule VulkanBaseContext::createShaderModule(const std::vector<char>& code)
{
...
void VulkanBaseContext::createFramebuffers()
{
...
void VulkanBaseContext::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
...
void VulkanBaseContext::createSemaphores()
{
...
void VulkanBaseContext::cleanupSwapChain()
{
...
void VulkanBaseContext::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
...
void VulkanBaseContext::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkDeviceSize srcOffset, VkDeviceSize dstOffset)
{
	VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void VulkanBaseContext::createVertexBuffer()
{
...
void VulkanBaseContext::createIndexBuffer()
{
...
uint32_t VulkanBaseContext::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void VulkanBaseContext::createDescriptorSetLayout()
{
...
void VulkanBaseContext::createUniformBuffer()
{
...
void VulkanBaseContext::updateUniformBuffer(uint32_t currentFrame)
{
...
void VulkanBaseContext::createDescriptorSets()
{
...
void VulkanBaseContext::createTextureImage()
{
...
void VulkanBaseContext::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) 
{
